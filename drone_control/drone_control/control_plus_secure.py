"""
Script Name: control_plus_secure.py
...
class ControlSecure(Node):

    def __init__(self):

        super().__init__('control_plus_secure')
        ...
        self.subscription = self.create_subscription(Empty,'takeoff',self.takeoff_callback,10)
    ...
    def joy_callback(self, msg):
            ...
            if button == 1: # Button is pressed
                ...
                if self.takeoff_state == 1:
                    if button_name == "SELECT":
                        self.get_logger().info(f'atterrissage')
        ...
        for i, axis in enumerate(axes):
            ...
            self.twist.linear.x = float(speed_x)
            self.twist.linear.y = float(speed_y)
            self.twist.linear.z = float(speed_z)
            self.twist.angular.z = float(speed_yaw)
